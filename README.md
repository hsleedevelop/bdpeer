...
- v0.5.24부터 macOS BLE central은 연결 타임아웃이나 service discovery 실패 후 stale peripheral을 정리하고 짧은 recovery scan을 예약해, 다음 검색에서 같은 피어를 다시 잡을 수 있도록 합니다. 필요 시 `BDPEER_DARWIN_BLE_STARTUP_SCAN_SECONDS=30 ./bdpeer`처럼 실행해 BLE powered-on 이후 startup scan을 더 오래 유지하는 실험을 할 수 있습니다.
