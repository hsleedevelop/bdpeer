...
- v0.5.23부터 macOS BLE central은 연결 타임아웃과 빈 service discovery 결과를 각각 1회 지연 재시도해, CoreBluetooth가 발견 직후 불안정한 상태를 반환하는 경우를 완화합니다. 필요 시 `BDPEER_DARWIN_BLE_STARTUP_SCAN_SECONDS=30 ./bdpeer`처럼 실행해 BLE powered-on 이후 startup scan을 더 오래 유지하는 실험을 할 수 있습니다.
